...
const StreakAnnouncer = () => {
  const { isOpen, closeModal } = useStreakAnnouncerModalStore(); // Destructure isOpen and closeModal

  const handleOverlayClick = useCallback((event: React.MouseEvent) => {  // useCallback here
    if (event.target === event.currentTarget) {
      closeModal();
    }
  }, [closeModal]); // Add closeModal to dependency array

  const handleCloseButtonClick = useCallback((event: React.MouseEvent) => { // useCallback here
    event.stopPropagation();
    closeModal();
  }, [closeModal]); // Add closeModal to dependency array

  if (!isOpen) return null; // Early return if not open, avoids unnecessary rendering
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h3>You are on a 3-day streak!</h3>
        <p>You are doing really great!</p>
      </div>
    </div>
  );
};
