...
const Contact = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage({ 
          type: 'error', 
          text: result.error || 'Sorry, there was an error sending your message. Please try again.' 
        });
      }
    } catch (error) {
  ...
  const getInputClassName = (fieldName: string) => {
    const baseClasses = "w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400 disabled:bg-gray-100 disabled:cursor-not-allowed";
    const errorClasses = validationErrors[fieldName] ? "border-red-500 focus:ring-red-400" : "border-gray-300";
          ...
          >
            ...
            <div>
              ...
              {validationErrors.inquiry_type && (
                <p className="text-red-500 text-sm mt-1">{validationErrors.inquiry_type}</p>
              )}
            </div>
            <div>
