...
export const usePracticeSession = () => {
    ...
    useEffect(() => {
        ...
        if (unattemptedQuestions.length > 0) {
            newQuestion = unattemptedQuestions[Math.floor(Math.random() * unattemptedQuestions.length)];
        } else if (allFilteredQuestions.length > 0) {
            newQuestion = allFilteredQuestions[Math.floor(Math.random() * allFilteredQuestions.length)];
        }
    ...
    return {
        data, difficulty, setDifficulty, selectedDomain, setSelectedDomain, mathDomains,
